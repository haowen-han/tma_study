...
def correctness_verification(height: int, width: int):
    ...
    with nvtx.annotate("test", color="blue"):
        my_torch_ext_v2.ops.double_buffer_add(process_input)
        my_torch_ext_v2.ops.origin_add(process_input_2)
    ...
    time_double_buffer = measure_operator(my_torch_ext_v2.ops.double_buffer_add, process_input)
    time_origin = measure_operator(my_torch_ext_v2.ops.origin_add, process_input_2)
...
if __name__ == "__main__":
    for height, width in ((2048 * 16, 128 * 33 * 4), (2048 * 16, 128 * 33 * 4 * 2)):
        for _ in range(10):
